...
import PoolMembers from '../../components/Pool/PoolMembers';
      ...
      <div className='flex flex-col gap-2'>
        <Card className='flex flex-row p-5 rounded-lg'>
            <Avatar className='w-24 h-24'>
              <AvatarImage src={poolData?.poolImage} alt={poolData?.poolName} className='object-cover'/>
              <AvatarFallback>{poolData?.poolName.charAt(0).toUpperCase()}{poolData?.poolName.charAt(1).toUpperCase()}</AvatarFallback>
            </Avatar>

            <div className='flex flex-col justify-center ml-5'>
              <h1 className='text-3xl font-bold tracking-tighter'>{poolData?.poolName}</h1>
            </div>
        </Card>
          ...
          <div className='flex flex-row gap-2 '>
              {
                isOwner && (
                  <>
                    <Link href={`/draft_admin?id=${poolData.poolID}`}><Button variant='default'>Draft</Button></Link>
